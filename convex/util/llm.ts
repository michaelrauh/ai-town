...
export const REQUIRED_OLLAMA_CHAT_MODEL = 'llama3.2:3b';
export const REQUIRED_OLLAMA_EMBEDDING_MODEL = 'mxbai-embed-large';
export const OLLAMA_EMBEDDING_DIMENSION = 1024;
...
function requireExactEnv(name: string, expected: string) {
  ...
  if (!actual) {
    throw new Error(`Missing ${name}. Set ${name}=${expected} for local Ollama.`);
  }
  if (actual !== expected) {
    throw new Error(`Invalid ${name}: ${actual}. This local build requires ${expected}.`);
  }
  return actual;
}

function rejectCloudProviderEnv() {
  const unsupported = [
    'OPENAI_API_KEY',
    'OPENAI_CHAT_MODEL',
  ...
  if (unsupported.length > 0) {
    throw new Error(
      `Cloud LLM configuration is disabled for this local build. Remove: ${unsupported.join(
        ', ',
      )}.`,
    );
  }
  if (process.env.LLM_PROVIDER && process.env.LLM_PROVIDER !== 'ollama') {
    throw new Error(`LLM_PROVIDER must be ollama for this local build.`);
  }
}

export function getLLMConfig(): LLMConfig {
  rejectCloudProviderEnv();
  if (EMBEDDING_DIMENSION !== OLLAMA_EMBEDDING_DIMENSION) {
    throw new Error(`EMBEDDING_DIMENSION must be ${OLLAMA_EMBEDDING_DIMENSION} for Ollama.`);
  }
  return {
    provider: 'ollama',
    url: process.env.OLLAMA_HOST ?? 'http://127.0.0.1:11434',
    chatModel: requireExactEnv('OLLAMA_MODEL', REQUIRED_OLLAMA_CHAT_MODEL),
    embeddingModel: requireExactEnv('OLLAMA_EMBEDDING_MODEL', REQUIRED_OLLAMA_EMBEDDING_MODEL),
    stopWords: ['<|eot_id|>'],
    apiKey: undefined,
  };
}
...
export async function tryPullOllama(model: string, error: string) {
  if (error.includes('try pulling')) {
    throw new Error(`Required Ollama model ${model} is missing. Run: ollama pull ${model}`);
  }
}
...
export async function fetchModeration(_content: string) {
  throw new Error(`Moderation is not available in the local-only Ollama build.`);
}
